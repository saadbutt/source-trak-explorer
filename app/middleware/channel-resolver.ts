...
interface ExtRequest extends Request {
	network: string;
	resolvedChannelGenesisHash?: string;
}
 ...
 */
export function channelResolver(platform: Platform) {
	return async (req: Request, res: Response, next: NextFunction) => {
		...
		console.log(
			`[Channel Resolver] Is Likely Channel Name: ${isLikelyChannelName}`
		);
		...
		if (isLikelyChannelName) {
			try {
				...
				console.log(
					`[Channel Resolver] Looking up channel: ${channelParam} in network: ${extReq.network}`
				);
				const channelResult = await dbCrudService.getChannelGenesisHashByName(
					extReq.network,
					channelParam
				);
				...
				if (channelResult && channelResult.channel_genesis_hash) {
					...
					console.log(
						`[Channel Resolver] Converted ${channelParam} to ${channelResult.channel_genesis_hash}`
					);
				} else {
					// Channel not found in database
					console.log(
						`[Channel Resolver] Channel '${channelParam}' not found in database`
					);
				...
				console.error(
					'[Channel Resolver] Error resolving channel name to genesis hash:',
					error
				);
			...
			console.log(
				`[Channel Resolver] Parameter ${channelParam} is already a genesis hash, skipping conversion`
			);
		}
